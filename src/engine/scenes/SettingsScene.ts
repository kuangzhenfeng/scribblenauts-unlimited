 ...
 *
 * 设计：用同一张“设置纸”承载分组内容，标题、当前状态、主要操作按固定顺序
 * 展开；平面分区替代旋转卡片堆叠。控件保持纸片质感但不依赖颜色传达状态，
 * 入场与状态变化尊重 prefers-reduced-motion。
 *
...
export class SettingsScene extends Phaser.Scene {
  ...
  private _injectStyles(): void {
    ...
    style.textContent = `
      .settings-overlay{color:#2b2b2b;font-family:${UI_FONT};}
      .set-shell{width:min(980px,100%);pointer-events:auto;}
      .set-card{
        position:relative;background:#f7f1e3;color:#2b2b2b;font-family:${UI_FONT};
        box-shadow:0 6px 14px rgba(60,40,20,0.18);border:1px solid rgba(43,43,43,0.16);
        border-radius:12px;padding:22px 24px 20px;width:100%;box-sizing:border-box;
        pointer-events:auto;animation:setCardIn .2s ease-out both;
      }
      .set-card-wide{grid-column:1 / -1}
      /* —— 区段头 —— */
      .set-head{display:flex;align-items:center;gap:12px;margin-bottom:18px;padding-bottom:14px;border-bottom:1px solid rgba(43,43,43,0.18)}
      .set-badge{width:42px;height:42px;border-radius:10px;display:grid;place-items:center;flex:none;color:#3d2200;background:#efe6cf;border:1px solid rgba(43,43,43,0.2)}
      .set-head-text{display:flex;flex-direction:column;gap:3px;min-width:0}
      .set-title{font-size:20px;font-weight:900;color:#2b2b2b;letter-spacing:0.03em}
      .set-desc{font-size:13px;color:#514b42;line-height:1.4}
      /* —— 通用行 —— */
      .set-row{display:flex;align-items:center;gap:14px;min-height:44px;margin-bottom:12px;pointer-events:auto}
      .set-row:last-child{margin-bottom:0}
      .set-label{font-size:15px;font-weight:800;color:#2b2b2b;min-width:88px}
      .set-value{font-size:14px;font-weight:900;color:#2b2b2b;min-width:42px;text-align:right;font-variant-numeric:tabular-nums}
      .set-sub{font-size:13px;font-weight:900;color:#514b42;margin:18px 0 8px}
      .set-sub:first-of-type{margin-top:0}
      .set-hint{font-size:12px;color:#514b42;line-height:1.45;margin-top:10px}
      /* —— 滑块：填充进度 + 自定义 thumb —— */
      .set-slider{
        -webkit-appearance:none;appearance:none;flex:1;height:44px;outline:none;cursor:pointer;
        background:linear-gradient(to right,#b56a0b 0%,#b56a0b var(--pct,10%),#d7ccb1 var(--pct,10%),#d7ccb1 100%);
      ...
      .set-slider::-webkit-slider-thumb{
        ...
        background:#2b2b2b;border:3px solid #f7f1e3;box-shadow:0 2px 5px rgba(0,0,0,0.25);
      ...
      .set-toggle{
        position:relative;width:58px;height:34px;border-radius:999px;flex:none;padding:0;
      ...
      .set-seg-btn{
        ...
        min-height:44px;background:#efe6cf;border:1px solid rgba(43,43,43,0.3);border-radius:8px;
      ...
      .set-seg-btn[aria-pressed="true"]{
        color:#fff8dd;background:#3d2200;border-color:#3d2200;
      }
      /* —— 难度档位卡片 —— */
      .set-tier{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}
      .set-tier-btn{
        min-height:64px;padding:11px 6px;border-radius:10px;cursor:pointer;text-align:center;
      ...
      .set-tier-btn[aria-pressed="true"]{
        background:#f0bd3c;border-color:#8a5a0a;color:#3d2200;
        box-shadow:0 3px 0 #a8771a;transform:translateY(-1px);
      }
      .set-tier-name{font-size:15px;font-weight:900}
      .set-tier-desc{font-size:11px;opacity:0.75;margin-top:2px}
      /* —— 词库分布表 —— */
      .set-worddist{margin-top:14px;padding:12px 14px;background:#efe6cf;border:1px solid rgba(43,43,43,0.18);border-radius:10px}
      ...
      /* —— 危险行 —— */
      .set-danger-row{display:flex;align-items:center;gap:14px;pointer-events:auto;padding:12px;background:#f5e8df;border:1px solid #c88768;border-radius:10px}
      ...
      /* —— 按钮 —— */
      .set-btn-ghost{
        display:flex;align-items:center;gap:8px;min-height:44px;font-family:${UI_FONT};font-size:16px;font-weight:900;
        color:#2b2b2b;background:#f7f1e3;border:1px solid rgba(43,43,43,0.4);border-radius:999px;
        padding:8px 18px;cursor:pointer;
        transition:transform .16s ease;
      }
      .set-btn-ghost:hover{transform:translateX(-2px)}
      .set-btn-danger{
        min-height:44px;font-family:${UI_FONT};font-size:15px;font-weight:900;color:#fff8dd;
        background:#a53918;border:1px solid #5a1a04;border-radius:8px;
        padding:9px 20px;cursor:pointer;flex:none;
        transition:transform .12s ease,filter .12s ease;
      }
      .set-btn-danger:hover{transform:translateY(-1px);filter:brightness(1.06)}
      .set-btn-try{
        min-height:44px;font-family:${UI_FONT};font-size:14px;font-weight:900;color:#2b2b2b;
      ...
      /* —— 标题区 —— */
      .set-header{display:flex;flex-direction:column;align-items:flex-start;gap:4px;margin:8px 0 18px}
      .set-h1{font-family:${UI_FONT};font-size:34px;font-weight:900;color:#2b2b2b;letter-spacing:0.02em}
      .set-subtitle{font-family:${UI_FONT};font-size:14px;color:#514b42}
      /* —— 焦点可见（a11y） —— */
      .settings-overlay :focus-visible{outline:3px solid #b56a0b;outline-offset:3px}
      /* —— 题目种子输入框 —— */
      .set-seed-input{
        font-family:${UI_FONT};font-size:14px;font-weight:900;color:#2b2b2b;
        background:#efe6cf;border:1px solid rgba(43,43,43,0.3);border-radius:8px;
        min-height:44px;padding:7px 12px;min-width:120px;max-width:180px;flex:none;
        transition:border-color .15s ease,box-shadow .15s ease;
      }
      .set-seed-input:focus{border-color:#b56a0b;box-shadow:0 0 0 3px rgba(181,106,11,0.2);outline:none}
      /* —— 入场动画 —— */
      @keyframes setCardIn{from{opacity:0;transform:translateY(8px) scale(0.99)}to{opacity:1;transform:translateY(0) scale(1)}}
      .set-cards > .set-card:nth-child(1){animation-delay:0ms}
      .set-cards > .set-card:nth-child(2){animation-delay:30ms}
      .set-cards > .set-card:nth-child(3){animation-delay:60ms}
      .set-cards > .set-card:nth-child(4){animation-delay:90ms}
      .set-status{min-height:18px;margin-top:8px;color:#3f7b3a;font-size:12px;line-height:1.4}
      .set-status[data-state="error"]{color:#8b2f18}
      @media (max-width:720px){
        .set-cards{grid-template-columns:1fr !important}
        .set-card-wide{grid-column:auto}
        .set-row{align-items:flex-start;flex-wrap:wrap}
        .set-label{flex:1;min-width:120px}
        .set-seg{width:100%}
        .set-seg-btn{flex:1}
        .set-tier{grid-template-columns:1fr}
        .set-seed-input{max-width:none;flex:1}
        .set-danger-row{align-items:flex-start;flex-wrap:wrap}
        .set-danger-mid{min-width:calc(100% - 58px)}
        .set-btn-danger{margin-left:58px}
      }
      /* —— reduced motion —— */
  ...
  private _buildOverlay(): void {
    ...
    this.overlay.style.cssText = [
      ...
      'align-items:center',
      'padding:max(78px,env(safe-area-inset-top) + 54px) max(16px,env(safe-area-inset-right)) max(28px,env(safe-area-inset-bottom)) max(16px,env(safe-area-inset-left))',
    ...
    cards.style.cssText = ['display:grid', 'grid-template-columns:repeat(2,minmax(0,1fr))', 'gap:16px', 'width:100%'].join(';');
  ...
  /** 分组工厂：统一纸片表面与标题层级，避免每个区段各自发明样式。 */
  private _card(_rotate: number, ...children: HTMLElement[]): HTMLDivElement {
  ...
  private _difficultyCard(): HTMLDivElement {
    ...
    tierGroup.setAttribute('aria-label', t('settings.tier'));
    ...
    const card = this._card(-0.4, this._head(ICON_SPARKLES, t('settings.difficulty'), t('settings.difficultyDesc')), wrap);
    card.classList.add('set-card-wide');
    return card;
  }
  ...
  private _dataCard(): HTMLDivElement {
    ...
    btn.addEventListener('click', async (ev) => {
      ...
      try {
        ...
        sfx.play('ui');
        status.textContent = loadSettings().language === 'zh' ? '存档已清除' : 'Save cleared';
      } catch {
        status.dataset.state = 'error';
        status.textContent = loadSettings().language === 'zh' ? '清除存档失败，请重试' : 'Could not clear save. Try again.';
      }
    });
    ...
    const card = this._card(0.5, this._head(ICON_TRASH, t('settings.data'), t('settings.dataDesc')), row);

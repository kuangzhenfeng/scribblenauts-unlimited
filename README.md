...
- 简易问答模式：面向手机竖屏的漫画词汇召唤玩法。标题页"简易问答"按钮进入，界面由顶栏 / 漫画委托条 / 画布 / 下屏输入台组成：顶栏含返回、回合、得分、连胜、换题与纯图标词频设置按钮；设置弹层支持 CEFR/词频标准、基础/进阶/大师难度、界面语言和随机种子重置（即时重建或刷新问答并持久化）；委托条悬浮在青蓝纸纹城市舞台背景上，下屏使用独立米白纸纹输入背景，键帽保持高对比实体纸面。随机生物出题（每题换一个生物），玩家输入形容词+名词生成物品，答对积分与连胜各+1并切下一题，答错连胜归零且保留当前题，物品持续 3 回合后消失；换题与切难度重置当前分数/连胜，最高分持久化。

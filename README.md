...
- 设置页：独立设置场景，提供界面语言（中文/English，切换即时生效并持久化，覆盖 UI 文案与游戏内 NPC 对话/物体名/题面）、音乐/音效音量滑块（实时生效）、主静音开关（同时控制音乐与音效）、屏幕方向（跟随设备/锁定横屏）、触屏控制（自动/开启/关闭）、难度档位（基础/进阶/大师）与难度定义方式（CEFR 等级/词频排名）切换（并展示两种标准下各档位的词库分布与题目分布）、题目种子查看/输入/刷新、清除存档，偏好 localStorage 持久化跨会话记忆
- 简易问答模式：面向手机竖屏的轻量问答玩法。标题页"简易问答"按钮进入，上屏为视差背景+随机提问生物（相机视口裁剪为键盘以上区域，游戏画面不被下屏键盘遮挡），下屏为定制化键盘（QWERTY+空格键+实时候选词+清空/退格/生成，圆角卡片质感，桌面端支持物理键盘）。随机生物出题（每题换一个生物），玩家输入形容词+名词（候选词合并补全形容词与名词，复用主游戏补全与解析路径）生成物品，答对积分+1并切下一题，物品持续 3 回合后消失，无限循环累计积分，最高分持久化。右上角"换题"按钮会话内重洗题序与提问生物序列（不碰存档种子，不影响主游戏题序，当前局分数清零、最高分保留）。
...
- **难度**：在设置页统一配置难度档（基础/进阶/大师）与分档标准（CEFR 等级 / 词频排名），进入关卡时直接生效，无需逐关选择。题库 1100+ 题，三层结构：情境多答案题（NPC 第一人称陈述困境如"好冷""天黑""饿""渴""怪物挡路"，不点名目标，2~6 个语义关联答案任一即过关）/ 词条覆盖单答案题（538 词条每个至少一道故事化题面）/ 形容词组合题（如"红色的鸟""凶猛的狗"，严格校验输错颜色/行为不算过关），按所选难度过滤后由存档种子随机抽取分配给本关 NPC。难度设置持久化到存档。题目种子可在设置页查看/输入/一键刷新，换种子重置所有关卡题目进度（保留关卡解锁与自制物体）。

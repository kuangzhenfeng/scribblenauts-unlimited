...
- **选关**：标题页"选择关卡"进入关卡选择界面，卡片网格展示全部关卡，按 Starite 门槛顺序解锁。每张卡片右下角提供「重置」按钮清空本关进度重玩，右上角「重置所有」按钮一键清空全部进度（挑战/Starite/碎片），自制物体保留。进度状态持久化到存档。
...
- **玩家控制**：WASD/方向键移动，空格/W/↑跳跃，F 拾取或骑乘面前的可骑乘物体，G 投掷/下坐骑，鼠标拖拽物体投掷；受伤/死亡后自动重生。触屏设备自动显示虚拟摇杆（左下）与跳跃/交互按钮（右下），可在设置中关闭。地图预生成的 NPC、树、石头等实体不阻挡玩家（穿行无阻），玩家生成的物体与之正常碰撞以驱动规则。
